...
#[cfg(not(target_family = "wasm"))]
type SparseBuffer = MmapMut;
#[cfg(target_family = "wasm")]
type SparseBuffer = Vec<u8>;

#[cfg(not(target_family = "wasm"))]
fn allocate_sparse_buffer(length: usize) -> Result<SparseBuffer, AsyncHttpRangeReaderError> {
...
#[cfg(not(target_family = "wasm"))]
fn spawn_streamer<F>(streamer: F)
where
...
{
    wasm_bindgen_futures::spawn_local(streamer);
}
...
impl AsyncHttpRangeReader {
    ...
    pub async fn from_range_response(
        ...
        let memory_map = allocate_sparse_buffer(complete_length as usize)?;
    ...
    pub async fn from_head_response(
        ...
        // Allocate a memory map to hold the data
        let memory_map = allocate_sparse_buffer(content_length as usize)?;

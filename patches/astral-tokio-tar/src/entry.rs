...
#[cfg(not(target_family = "wasm"))]
use tokio::fs::{self, remove_file, OpenOptions};
use tokio::io::{self, AsyncRead as Read, AsyncReadExt, AsyncSeekExt, AsyncWriteExt};
#[cfg(target_family = "wasm")]
use uv_vfs::fs::tokio::{self as fs, remove_file, OpenOptions};
...
impl<R: Read + Unpin> Entry<R> {
    ...
    pub async fn unpack_in<P: AsRef<Path>>(&mut self, dst: P) -> io::Result<Option<PathBuf>> {
        let dst = canonicalize(dst.as_ref())?;
...
impl<R: Read + Unpin> EntryFields<R> {
    ...
    async fn unpack_in(
        ...
        if cfg!(debug_assertions) {
            let canon_target = canonicalize(dst)?;
    ...
    async fn unpack(&mut self, target_base: Option<&Path>, dst: &Path) -> io::Result<Unpacked> {
        ...
        if self.preserve_mtime {
            if let Some(mtime) = get_mtime(&self.header)? {
                ...
                #[cfg(target_family = "wasm")]
                f.set_modified(mtime).await.map_err(|e| {
                    TarError::new(format!("failed to set mtime for `{}`", dst.display()), e)
                })?;
            }
        }
    ...
    async fn validate_inside_dst(&self, dst: &Path, file_dst: &Path) -> io::Result<()> {
        ...
        let canon_parent = canonicalize(file_dst).map_err(|err| {
            Error::new(
                err.kind(),

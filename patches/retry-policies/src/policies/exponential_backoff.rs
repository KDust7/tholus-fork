...
/// `SystemTime::now` panics on `wasm32-unknown-unknown`, where the standard library has no clock.
/// `UNIX_EPOCH` is a constant and adding a `Duration` to it is supported on every target, so the
/// browser's clock is read through `web_time` and a std `SystemTime` is rebuilt from it. Callers
/// keep taking and returning `std::time::SystemTime`, which is what `RetryPolicy` is defined over.
...
impl RetryPolicy for ExponentialBackoff {
    fn should_retry(&self, _request_start_time: SystemTime, n_past_retries: u32) -> RetryDecision {
        if self.too_many_attempts(n_past_retries) {
            ...
            let execute_after = now() + clip(jittered_wait_for, self.max_retry_interval);

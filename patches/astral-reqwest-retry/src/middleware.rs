    ...
    async fn execute_with_retry<'a>(
        ...
        loop {
            ...
            break if n_past_retries > 0 {
                ...
                match result {
                    Ok(response) => Ok(record_retry_count(response, n_past_retries)),
...
/// Record how many retries a response took.
///
/// `wasm32` targets reach the browser `fetch` API, whose `Response` carries no extension map, so
/// the count is dropped there rather than the retry loop being disabled.
#[cfg(not(target_arch = "wasm32"))]
fn record_retry_count(mut response: Response, retries: u32) -> Response {
    response.extensions_mut().insert(RetryCount::new(retries));
    response
}

#[cfg(target_arch = "wasm32")]
fn record_retry_count(response: Response, _retries: u32) -> Response {
    response
}
